...
def _handle_save(_):
    ...
    if df is None or df.empty:
        with status_output:
            clear_output()
            print("⚠️ Upload a CSV before saving.")
        return
    ...
    if not prompt_col:
        with status_output:
            clear_output()
            print("⚠️ Select which column contains the prompt text.")
        return
    ...
    if prompts.empty:
        with status_output:
            clear_output()
            print("⚠️ No valid prompt rows were found after trimming empty values.")
        return
